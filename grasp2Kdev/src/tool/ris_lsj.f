...
*
      ...
      INTEGER MAX_STRING_LENGTH,MAX_STRING_LENGTH_TOT,N,NF0CORR
*
...
! Start to read the complete file to search for F0CORR to see which case
! we have
...
CPJ        IF ((ans.eq.'n').or.(ans.eq.'N')) GOTO 73
   ...
   73 CONTINUE
     ...
     &     STATUS = 'UNKNOWN')
      ...
      IF (NF0CORR.EQ.1) THEN

         WRITE(80,402) 'Energy',DUMMY(1:N),'State',
     & '    J   P      KNMS(a.u.)      KSMS(a.u.)    F0CORR(GHz/fm^2)'
...
! Position for reading electron density at the nucleus
...
! Position for reading F0CORR

         DO I = 1,15+2*IMaxCount
            READ (30,'(A)') RECORD
         END DO
...
! Now read either the density of F0CORR
