...
* --- Give the number of orbitals that will not be part of the CC or CV
*     CSFs

      write(*,*) ' Give the number of non-relativistic orbitals not'
      write(*,*) ' to be allowed in the construction of the CSFs in'
      write(*,*) ' the core-core or core-valence PCF lists         '
      read(*,*) norbexclude
      do i = 1,norbexclude
        write(*,*) ' Give orbital',i,'in non-relativistic notation'
        read(*,'(a)') orbexclude(i)
      end do
...
* --- Check for CSFs containing orbitals from a user defined set

        nfound = 0
        do ij = 1,norbexclude
          nfound = index(trim(line1),orbexclude(ij))  
          if (nfound.ne.0) then 
            exit
          end if
        end do

* --- Write CSF to correct PCF file. Do not write CSFs to
*     core-core or core-valence PCF file if CSF contain orbitals
*     from the user defined set

        if ((j.le.ncore).and.(k.le.ncore).and.(nfound.eq.0)) then
          write(10+j+40*k,'(a)') trim(line1)
          write(10+j+40*k,'(a)') trim(line2)
          write(10+j+40*k,'(a)') trim(line3)
        end if

        if ((j.le.ncore).and.(k.gt.ncore).and.(nfound.eq.0)) then
