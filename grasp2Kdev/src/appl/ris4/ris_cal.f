...
Cww     :        PINDTE,PVALTE
      ...
      LOGICAL LFORDR,LTRANS,LVP,LSE,LNMS,LSMS,YES2
*
...
*     C*      ENDIF
      ...
      PRINT *,' Compute higher order field shift electronic factors?'
      YES2 = GETYN ()
*
...
!         endif
 ...
 14   CONTINUE
     ...
     :           LABP((IASPAR(I)+3)/2),RHO
 16      CONTINUE
         WRITE (24,317)
         DO 17 I = 1,NVEC
            CALL EDENSITYFIT(R,DENS1VEC(I,:),Z,PARM,NRNUC,FMAT,RHO,RES)  ! JE ADD
            WRITE (24,324) IVEC(I),LABJ(IATJPO(I)),
     :           LABP((IASPAR(I)+3)/2),FMAT(1),FMAT(2),
     :           FMAT(3),FMAT(4),RES
 17      CONTINUE
         WRITE (24,327)
         DO 19 I = 1,NVEC
...
!     Field shift values
 334  FORMAT (1X,I3,5X,2A4,3X,1D20.10)
...
!     Electronic field shift header
 327  FORMAT (//' Level  J Parity  Field shift (corrected for varying de
     :nsity inside nucleus)'
     :     //24X,
     :     'F0CORR (GHz/fm^2)')
