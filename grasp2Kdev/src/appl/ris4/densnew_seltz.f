...
*
      ...
      DIMENSION DINT1VEC(NNNW,NNNW,NRNUC),ELEMNT1VEC(NRNUC)                   ! JE ADD
      DIMENSION CONTRI1VEC(NRNUC),DENS1VEC(NVEC,NRNUC)                        ! JE ADD
*
...
*
            ...
            if (mod(IC,100).eq.0) then
               PRINT*, 'Columns '//CNUM(1:LCNUM)//' complete;'
            end if
*     
            ITJPOC = ITJPO (IC)
...
CGG   CALL TNSRJJ (KA,IOPAR,IC,IR,IA,IB,TSHELL)
                        ...
                        DO 8 IA = 1,NW
                           IF (ABS (TSHELL(IA)) .GT. CUTOFF) THEN
                              ...
                              DO 21 L = 2,NRNUC ! JE ADD
                                 ELEMNT1VEC(L) = ELEMNT1VEC(L) +
     :                                DINT1VEC(IA,IA,L)*TSHELL(IA)
 21                           CONTINUE
 ...
 23                           CONTINUE
                     ...
                     ENDIF
                  ENDIF
                  DO 9 J = NEVBL(II)+1,NEVBL(II+1) ! JE ADD
                     LOC = (J-1)*NCF
                     CONTRI1 = EVEC(IC+LOC)*EVEC(IR+LOC)*ELEMNT1
                     CONTRI1VEC(:) =
     :                    EVEC(IC+LOC)*EVEC(IR+LOC)*ELEMNT1VEC(:) ! JE ADD
                     ...
                     IF (IR.NE.IC) THEN
                        ...
                        CONTRI1VEC(:) = 2.0D 00 * CONTRI1VEC(:) ! JE ADD
